...
POSENET_SHARED_LIB = os.path.join('/dependencies/posenet_lib', os.uname().machine, 'posenet_decoder.so')
...
class PoseEngine():
  ...
  def DetectPosesInImage(self, img):
    ...
    resized_image = cv2.resize(img,
        (self._input_width, self._input_height), interpolation= cv2.INTER_LINEAR)
